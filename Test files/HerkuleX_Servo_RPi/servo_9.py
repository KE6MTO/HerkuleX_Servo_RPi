...
def calcLegValues(step_low,step_high,start_x,start_y,end_x,end_y):
	debug = "true"
	if debug != "false": #show debug information
		print " "
	...
	step_end_high_coxa_servo_value = 0
	step_end_high_femur_Servo_value = 0
	step_end_high_tibia_Servo_value = 0
	step_start_coxa_servo_value = 0
	step_start_femur_Servo_value = 0
	step_start_tibia_Servo_value = 0
...
def	walkTest():
	...
	print " "
	print "Step start"
	m = (step_start_y-coord_Y1)/float((step_start_x-coord_X1))
	print "  M Value: " + str(m)
	targetAngle=(math.atan(m)*180)/math.pi
	print "  Coxa Angle: " + str(targetAngle)

	print "Coxa Length= " + str(LENGTH_COXA)
	updated_base_x_coord= coord_X1 + (LENGTH_COXA * math.cos((targetAngle*math.pi)/180))
	print "  Femur X Coord: " + str(updated_base_x_coord)
	updated_base_y_coord= coord_Y1 + (LENGTH_COXA* math.sin((targetAngle*math.pi)/180))	
	print "  Femur Y Coord: " + str(updated_base_y_coord)
	targetDistance=math.sqrt(math.pow(step_start_x-updated_base_x_coord, 2)+math.pow(step_start_y-updated_base_y_coord, 2))
	print "  Tibia distance: " + str(targetDistance)
	#legServoValues = servoCal2(step_down_height,targetDistance) #bodyHeight,footDistance		
	adjustedTargetAngle=targetAngle-45
	startCoxaServo=512+(adjustedTargetAngle/.325) #Leg Coxa
	print "Coxa Servo Value: " + str(startCoxaServo)
	startLegServoValues = ikLeg(targetDistance,step_down_height)
	
	print " "
	print "Step end"
	m = (step_end_y-coord_Y1)/float((step_end_x-coord_X1))
	print "  M Value: " + str(m)
	targetAngle=(math.atan(m)*180)/math.pi
	print "  Coxa Angle: " + str(targetAngle)
	...
	updated_base_y_coord= coord_Y1 + (LENGTH_COXA* math.sin((targetAngle*math.pi)/180))	
	print "  Femur Y Coord: " + str(updated_base_y_coord)
	targetDistance=math.sqrt(math.pow(step_end_x-updated_base_x_coord, 2)+math.pow(step_end_y-updated_base_y_coord, 2))
	print "  Tibia distance: " + str(targetDistance)
	#legServoValues = servoCal2(step_down_height,targetDistance) #bodyHeight,footDistance		
	adjustedTargetAngle=targetAngle-45
	endCoxaServo=512+(adjustedTargetAngle/.325) #Leg Coxa
	print "Coxa Servo Value: " + str(endCoxaServo)
	endLegServoValues = ikLeg(targetDistance,step_down_height)	
	while True :
		time.sleep(.6)
		print "start"
		singleServoMove(leg1Servos[0],startCoxaServo,60,"BLUE")
		singleServoMove(leg1Servos[1],startLegServoValues[0],60,"GREEN")
		singleServoMove(leg1Servos[2],startLegServoValues[1],60,"GREEN")
		time.sleep(.6)
		print "end"
		singleServoMove(leg1Servos[0],endCoxaServo,60,"BLUE")
		singleServoMove(leg1Servos[1],endLegServoValues[0],60,"GREEN")
		singleServoMove(leg1Servos[2],endLegServoValues[1],60,"GREEN")
...
def	legSwingTest():
	...
	for x in range(0, 4):
		...
		#legServoValues = servoCal2(step_down_height,targetDistance) #bodyHeight,footDistance		
		adjustedTargetAngle=targetAngle-45
		CoxaServo=512+(adjustedTargetAngle/.325) #Leg Coxa
		print "Coxa Servo Value: " + str(CoxaServo)
		
		
		legServoValues = ikLeg(targetDistance,step_down_height)
		if legServoValues == "fail":
			print "No solution"
		else:
			#legServoValues = ikLeg(50,110)
			singleServoMove(leg1Servos[0],CoxaServo,60,"BLUE")
			singleServoMove(leg1Servos[1],legServoValues[0],60,"GREEN")
			singleServoMove(leg1Servos[2],legServoValues[1],60,"GREEN")
