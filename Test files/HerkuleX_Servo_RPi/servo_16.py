...
leg1CoxaBaseX=150 #150mm
leg1CoxaBaseY=150 #150mm

leg2CoxaBaseX=150 #150mm
leg2CoxaBaseY=-150 #150mm

leg3CoxaBaseX=-150 #150mm
leg3CoxaBaseY=-150 #150mm

leg4CoxaBaseX=-150 #150mm
leg4CoxaBaseY=150 #150mm
...
def calcLegValues2(leg,height,x,y):
	debug = "true"
	if debug == "true": #show debug information
		print "  Cal based on leg=", leg, "height=", height, "x=", x, "y=", y

	if leg == 1:
		legBase_x= leg1CoxaBaseX + x
		legBase_y= leg1CoxaBaseY + y
		m = (y)/float((x))
		targetAngle=(math.atan(m)*180.0)/math.pi
		z = (targetAngle*math.pi)/180
		updated_base_x_coord= (LENGTH_COXA * math.cos(z))+leg1CoxaBaseX
		updated_base_y_coord= (LENGTH_COXA* math.sin(z))+leg1CoxaBaseY
		targetDistance=math.sqrt(math.pow(legBase_x-updated_base_x_coord, 2)+math.pow(legBase_y-updated_base_y_coord, 2))	
		if debug == "true": #show debug information
			print "  M Value: " + str(m)
			print "  Z Value: " + str(z)
			print "  Updated X: " + str(legBase_x)
			print "  Updated Y: " + str(legBase_y)
			print "  Coxa Angle: " + str(targetAngle)
			print "  Coxa LENGTH: " + str(LENGTH_COXA)
	...
	if leg == 2:
		legBase_x= leg2CoxaBaseX + x
		legBase_y= leg2CoxaBaseY + y
		m = (y)/float((x))
		targetAngle=(math.atan(m)*180.0)/math.pi
		z = (targetAngle*math.pi)/180
		updated_base_x_coord= (LENGTH_COXA * math.cos(z))+leg2CoxaBaseX
		updated_base_y_coord= (LENGTH_COXA* math.sin(z))+leg2CoxaBaseY
		targetDistance=math.sqrt(math.pow(legBase_x-updated_base_x_coord, 2)+math.pow(legBase_y-updated_base_y_coord, 2))	
		if debug == "true": #show debug information
			print "  M Value: " + str(m)
			print "  Z Value: " + str(z)
	...
	if leg == 3:
		legBase_x= leg3CoxaBaseX + x
		legBase_y= leg3CoxaBaseY + y
		m = (y)/float((x))
		targetAngle=(math.atan(m)*180.0)/math.pi
		z = (targetAngle*math.pi)/180
		updated_base_x_coord= ((LENGTH_COXA * math.cos(z))*-1)+leg3CoxaBaseX
		updated_base_y_coord= (LENGTH_COXA* math.sin(z)*-1)+leg3CoxaBaseY
		targetDistance=math.sqrt(math.pow(legBase_x-updated_base_x_coord, 2)+math.pow(legBase_y-updated_base_y_coord, 2))	
		if debug == "true": #show debug information
			print "  M Value: " + str(m)
			print "  Z Value: " + str(z)
	...
	if leg == 4:
		legBase_x= leg4CoxaBaseX + x
		legBase_y= leg4CoxaBaseY + y
		m = (y)/float((x))
		targetAngle=(math.atan(m)*180.0)/math.pi
		z = (targetAngle*math.pi)/180
		updated_base_x_coord= ((LENGTH_COXA * math.cos(z))*-1)+leg4CoxaBaseX
		updated_base_y_coord= ((LENGTH_COXA* math.sin(z))*-1)+leg4CoxaBaseY
		targetDistance=math.sqrt(math.pow(legBase_x-updated_base_x_coord, 2)+math.pow(legBase_y-updated_base_y_coord, 2))	
		if debug == "true": #show debug information
			print "  M Value: " + str(m)
			print "  Z Value: " + str(z)
	...
	startLegServoValues = ikLeg(targetDistance,height)
	
	coxa_servo_value = targetAngle
	femur_Servo_value = startLegServoValues[0]
	tibia_Servo_value = startLegServoValues[1]
	
	return (coxa_servo_value,femur_Servo_value,tibia_Servo_value) 	
...
def	walkTest():
	...
	print "Leg 1"
	leg1Values = calcLegValues2(1,110,40,80) #height, x, y

	print "Leg 2"
	leg2Values = calcLegValues2(2,110,40,-80) #height, x, y
	
	print "Leg 3"
	leg3Values = calcLegValues2(3,110,-40,-80) #height, x, y
	
	print "Leg 4"
	leg4Values = calcLegValues2(4,110,-40,80) #height, x, y
	
	print "-----> Move legs now"
	singleServoMove(leg1Servos[0],512-((leg1Values[0]-45.0)/.325),step_cycle_time,"RED")
	...
	singleServoMove(leg2Servos[0],512-((leg2Values[0]+45.0)/.325),step_cycle_time,"Blue")
	...
	singleServoMove(leg3Servos[0],512-((leg3Values[0]-45.0)/.325),step_cycle_time,"Blue")
	...
	singleServoMove(leg4Servos[0],512-((leg4Values[0]+45.0)/.325),step_cycle_time,"Blue")
...
def	walkTest2():
	...
	for i in range(0,3):
		...
		if i == 1:
			print "Leg 1"
			leg1Values = calcLegValues2(1,60,40,80) #height, x, y

			print "Leg 2"
			leg2Values = calcLegValues2(2,60,40,-80) #height, x, y

		if i == 2:
			print "Leg 1"
			leg1Values = calcLegValues2(1,110,40,20) #height, x, y

			print "Leg 2"
			leg2Values = calcLegValues2(2,110,40,-20) #height, x, y
